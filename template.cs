...
namespace Template
{
    public class OpenTKApp : GameWindow
    {
        ...
        protected override void OnResize(ResizeEventArgs e)
        {
            ...
            app.screen = new(e.Width, e.Height);
            app.target = new RenderTarget(app.screen.width, app.screen.height);
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            if (keyboard[Keys.D1]) app.play = true;
            if (keyboard[Keys.D2]) app.play = false;
            app.camera.HandleInput(keyboard, (float)e.Time);
            app.Tick((float)e.Time, keyboard); // Pass deltaTime and keyboardState to the Tick method
        }
        protected override void OnRenderFrame(FrameEventArgs e)
        {
            ...
            if (app != null) app.Tick((float)e.Time, KeyboardState); // Pass deltaTime and keyboardState to the Tick method
            if (terminated)
            {
                Close();
                return;
            ...
            if (app != null)
            {
                ...
                else
                {
                    if (quad != null && screenShader != null)
                        quad.Render(screenShader, screenID, (app.screen.width, app.screen.height));
                }
